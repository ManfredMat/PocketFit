...
const WeekTable = (props) => {
  ...
  const saveChanges = async () => {
    ...
    await axios.post(routineRoute, dayIds);
    if (props.id)
      await axios.put(`http://localhost:3001/api/users/${props.id}`, {
        customRoutine: true,
      });
    ...
    for (const key in weekChanges) {
      ...
      for (const key2 in weekChanges[key].blocks) {
        let response = await axios.post(
          "http://127.0.0.1:3001/api/blocks/",
          weekChanges[key].blocks[key2]
        );

        routinesIds.push(response.data.id);
      }

      weekIds[key] = await postRoutine(
        routinesIds,
        weekChanges[key].dayRoutine
      );
    }
    return weekIds;
  };
  ...
  const postRoutine = async (blocksIds, dayRoutine) => {
    const response = await axios.post("http://127.0.0.1:3001/api/routines", {
      ...dayRoutine,
      blocks: blocksIds,
    });
                    ...
                    block1: response[key].blocks[0]
                      ? {
                          ...
                          exercises: response[key].blocks[0].exercises
                            ? response[key].blocks[0].exercises.map(
                                (exercise) => {
                                  return {
                                    reps: exercise[1],
                    ...
                    block2: response[key].blocks[1]
                      ? {
                          ...
                          exercises: response[key].blocks[1].exercises
                            ? response[key].blocks[1].exercises.map(
                                (exercise) => {
                                  return {
                                    reps: exercise[1],
                                    description: exercise[2],
                    ...
                    block3: response[key].blocks[2]
                      ? {
                          ...
                          exercises: response[key].blocks[2].exercises
                            ? response[key].blocks[2].exercises.map(
                                (exercise) => {
                                  return {
                                    reps: exercise[1],
                                    description: exercise[2],
      ...
      <HeaderConteiner>
        ...
        <h1>
          ...
          <span style={{ fontWeight: "400" }}>
            {userName ? `de ${userName}` : "General"}
          </span>
        </h1>
        <SaveDeleteToggle onClick={() => setDisableButtons(!disableButtons)}>
          Editar
        </SaveDeleteToggle>
      </HeaderConteiner>
      <DayContainer>
